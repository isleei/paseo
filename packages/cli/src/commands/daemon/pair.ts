...
async function resolveDaemonPairingOffer(
  ...
  try {
    ...
    if (serverInfo?.features?.daemonStatusRpc !== true) {
      throw new Error("Update the Paimon daemon before pairing from this command.");
    }
    ...
    if (!offer.relayEnabled && enableRelay) {
      if (serverInfo.features.relayConfig !== true) {
        throw new Error("Update the Paimon daemon before enabling relay from this command.");
      }
...
export async function confirmRelayPairing(): Promise<boolean> {
  log.message("Your connection is end-to-end encrypted. Paimon cannot read your code or messages.");
