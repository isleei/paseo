...
const NATIVE_COMPACT_BADGE_WIDTH: Array<{
  ...
  {
    label: "12 个任务",
    pillSnapshot: { activeSessionCount: 0, sessionCount: 12 },
    layoutWidth: 22,
    renderedWidth: 22,
  },
  {
    label: "1/2",
    pillSnapshot: { activeSessionCount: 1, sessionCount: 2 },
    layoutWidth: 30,
    renderedWidth: 30,
  },
  {
    label: "9/9",
    pillSnapshot: { activeSessionCount: 9, sessionCount: 9 },
    layoutWidth: 30,
    renderedWidth: 30,
  },
  {
    label: "1/12",
    pillSnapshot: { activeSessionCount: 1, sessionCount: 12 },
    layoutWidth: 37,
    renderedWidth: 33,
  },
  {
  ...
  {
    label: "10/100",
    pillSnapshot: { activeSessionCount: 10, sessionCount: 100 },
    layoutWidth: 51,
    renderedWidth: 45,
  },
  {
    label: "99/99",
    pillSnapshot: { activeSessionCount: 99, sessionCount: 99 },
    layoutWidth: 44,
    renderedWidth: 39,
  },
  {
    label: "123/456",
    pillSnapshot: { activeSessionCount: 123, sessionCount: 456 },
    layoutWidth: 58,
    renderedWidth: 51,
  },
];
...
describe("getAgentIslandCompactBadgeWidth", () => {
  it("与 Swift PillBadge.intrinsicWidth 逐值一致(两侧公式必须同步)", () => {
    for (const { label, pillSnapshot, layoutWidth } of NATIVE_COMPACT_BADGE_WIDTH) {
      expect(getAgentIslandCompactBadgeWidth(pillSnapshot), `${label} 与 Swift 侧不一致`).toBe(
        layoutWidth,
      );
    }
  });
  ...
  it("标称宽度覆盖 SwiftUI 实绘宽度(否则徽标仍会被裁)", () => {
    for (const { label, pillSnapshot, renderedWidth } of NATIVE_COMPACT_BADGE_WIDTH) {
      ...
      expect(nominal, `${label} 标称 ${nominal} < 实绘 ${renderedWidth}`).toBeGreaterThanOrEqual(
        renderedWidth,
      );
    }
  });
  ...
  it("单个计数在 22pt 最小宽度内不变宽,超出后同样按位数增长", () => {
    ...
    expect(
      getAgentIslandCompactBadgeWidth({ activeSessionCount: 0, sessionCount: 1234 }),
    ).toBeGreaterThan(22);
...
describe("getAgentIslandDefaultContentWidth: 刻痕机为计数徽标预留宽度", () => {
  ...
  it("预留量等于 (徽标宽 + inset 上界) * 2", () => {
    ...
    ).toBe(notchWidth + (badgeWidth + AGENT_ISLAND_COMPACT_HARDWARE_BADGE_RESERVED_INSET) * 2);
  });
...
describe("snapAgentIslandCompactHardwareContentWidth", () => {
  ...
  it("计数变多不会把此前停在 basic 的持久化宽度判成 hidden(灵动岛不塌缩)", () => {
    ...
    for (const pillSnapshot of [
      ...
      expect(
        resolved,
        `${pillSnapshot.activeSessionCount}/${pillSnapshot.sessionCount} 塌缩到了 ${resolved}`,
      ).not.toBe(notchWidth);
