import {
  DEFAULT_PASEO_ISLAND_TOOL_WORDING,
  isPaseoTurnContinuationBoundaryEvent as isTurnContinuationBoundaryEvent,
  parsePaseoReconnectAttemptMessage as parseReconnectAttemptMessage,
  stripPaseoTrailingPathSeparators as stripTrailingPathSeparators,
  type PaseoIslandAgentEvent as AgentEvent,
  type PaseoIslandInteractionRequest as InteractionRequest,
  type PaseoIslandToolWording as ToolRowWording,
} from "./island-event-types.js";

const DEFAULT_TOOL_ROW_WORDING = DEFAULT_PASEO_ISLAND_TOOL_WORDING;

import { LIVE_TASK_PRIORITY, liveTaskPriorityRank } from "./live-task-priority.js";

import { formatIslandToolDetail } from "./tool-detail.js";
import {
...
export function setAgentIslandAppFocused(
...
export function setAgentIslandVisibleSession(
  ...
  const visibleSessionsChanged =
    previousSessionIds.size !== state.visibleSessionIds.size ||
...
export function setAgentIslandMeasuredContentHeight(
  ...
  const next = Math.max(
    0,
    Math.min(AGENT_ISLAND_MEASURED_HEIGHT_MAX, Math.ceil(measuredContentHeight)),
  );
...
export function setAgentIslandHovered(
  state: AgentIslandState,
  hovered: boolean,
  now: number,
): boolean {
...
export function setAgentIslandPointerZones(
  ...
  return (
    wasPointerInside !== isPointerInsideIsland(state) ||
...
export function setAgentIslandLayoutDragActive(state: AgentIslandState, active: boolean): boolean {
...
export function requestAgentIslandManualExpand(
  state: AgentIslandState,
  displayId?: number | null,
): boolean {
  if (state.layoutDragActive) return false;
  const nextDisplayId =
    typeof displayId === "number" && Number.isFinite(displayId) ? displayId : state.hoverDisplayId;
  const changed =
    !state.hoverExpanded ||
    state.hoverIntentAt !== null ||
    state.collapseAt !== null ||
...
function restoreSessionScopedReveal(
  ...
  const wantsActive =
    session.revealUntil != null && session.revealUntil > now && session.deferredReveal !== true;
...
// Ported verbatim from Cindy; the event reducer is intentionally a single
// function so terminal-state transitions stay in one place.
...
export function applyAgentIslandEvent(
  ...
  if (event.type === "text") {
    ...
    if (
      line &&
      !session.messagePreviewQueue.some((queued) => queued.id === line.id) &&
  ...
  if (event.type === "tool_use") {
    ...
    const toolDescription = toolName
      ? formatIslandToolDetail(
          toolName,
          toolInput,
          { wording: state.toolWording },
          data ?? undefined,
        )
  ...
  if (event.type === "done") {
    ...
    if (session.pendingInteractionIds.size > 0) {
      ...
      session.detail = session.interactionKind
        ? (session.pendingInteractionDetails.get([...session.pendingInteractionIds][0] ?? "") ??
          session.detail)
        : session.detail;
  ...
  if (event.type === "error") {
    ...
    } else if (typeof data?.willRetry === "boolean") {
      isTerminal = !data.willRetry;
    }
    if (!isTerminal) {
    ...
    if (data?.reason === "output-limit") {
      session.detail = state.strings.outputLimit;
    } else if (isToolLoopError) {
      session.detail = state.strings.error;
    } else if (typeof data?.message === "string" && data.message.trim()) {
      session.detail = data.message.trim();
    } else {
      session.detail = "";
    }
    ...
    session.completionAllowedAfterTerminalError =
      options.allowCompletionAfterTerminalError === true;
    session.completedUntil = null;
    ...
    requestAttentionReveal(state, session, now, AGENT_ISLAND_ERROR_REVEAL_DWELL_MS, {
      forceUnread: true,
    });
...
export function applyAgentIslandInteractionRequest(
  ...
  session.pendingInteractionDetails.set(
    request.requestId,
    detailForInteraction(request, state.toolWording),
  );
  markSessionRunning(state, session, now);
...
export function requestAgentIslandSessionFocus(
  ...
  return (
    completionDismissed ||
    previousSessionId !== state.pendingFocusSessionId ||
    previousUntil !== state.pendingFocusUntil
  );
}
...
export function isAgentIslandPendingFocusAck(
  ...
  if (!state.pendingFocusSessionId || !state.pendingFocusUntil || state.pendingFocusUntil <= now)
    return false;
...
export function dismissAgentIslandActiveReveal(state: AgentIslandState, now: number): boolean {
  ...
  const clickProtectedUntil =
    state.expandedProtectUntil === null
      ? null
      : state.expandedProtectUntil -
        AGENT_ISLAND_EXPANDED_MIN_DWELL_MS +
        AGENT_ISLAND_OUTSIDE_CLICK_PROTECTION_MS;
  if (
    clickProtectedUntil !== null &&
    clickProtectedUntil > now &&
...
export function pruneAgentIslandSessions(state: AgentIslandState, now: number): void {
  ...
  const preserveExpiredTransient =
...
function updateExpandedProtection(
  ...
  const expandedIdentityChanged =
    state.lastDisplayMode === "expanded" &&
...
function updateToolDetailLifecycle(state: AgentIslandState, now: number): void {
  for (const session of state.sessions.values()) {
    if (session.currentToolUseId || !session.toolDetailUntil || session.toolDetailUntil > now)
      continue;
    session.toolDetailUntil = null;
...
function completionRevealDwellMs(session: AgentIslandSessionState, now: number): number {
  ...
  return Math.max(
    ...
    remainingPreviewMs + AGENT_ISLAND_REVEAL_DWELL_MS,
  );
}
...
function pendingFocusNavigationExpiresAt(state: AgentIslandState): number | null {
  return state.pendingFocusUntil === null
    ...
    : state.pendingFocusUntil -
        AGENT_ISLAND_FOCUS_VERIFY_TIMEOUT_MS +
        AGENT_ISLAND_FOCUS_NAVIGATION_TIMEOUT_MS;
}
...
function promoteNextTransientReveal(
  ...
  if (
    manualExpanded ||
    hasBlockingSession(state, now) ||
    hasDisplayableDismissedBlockingSession(state, now)
  )
    return;
...
function getActiveTransientSession(
  state: AgentIslandState,
  now: number,
): AgentIslandSessionState | null {
...
function getActiveBlockingReveal(
  state: AgentIslandState,
  now: number,
): AgentIslandSessionState | null {
  return (
    Array.from(state.sessions.values())
      .filter(
        (session) => isBlockingSession(session) && !isBlockingRevealSuppressed(state, session),
      )
...
function shouldSmartSuppressSession(
  state: AgentIslandState,
  session: AgentIslandSessionState,
): boolean {
...
function orderSessionsForAutomaticTransientStack(
  ...
  if (manualExpanded || !activeTransient || sortedSessions.some(isBlockingSession))
    return sortedSessions;
  return [
    activeTransient,
...
function buildDisplayDecision(
  ...
  const stableCompactCurrent = selectStableCompactCurrent(
    state,
    orderedSessions,
    compactCurrent,
    now,
  );
  const compactSmartSuppressed =
    Boolean(stableCompactCurrent.deferredReveal) ||
    (isBlockingSession(stableCompactCurrent) &&
      isBlockingRevealSuppressed(state, stableCompactCurrent));
  return {
...
function selectStableCompactCurrent(
  ...
  const previous = state.compactCurrentSessionId
    ? (orderedSessions.find((session) => session.sessionId === state.compactCurrentSessionId) ??
      null)
    : null;
...
function orderSessionsForDecisionSurface(
  ...
  return [current, ...orderedSessions.filter((session) => session.sessionId !== currentId)];
}
...
function getDisplaySessionsForSurface(
  ...
  const transientSessions = orderedSessions.filter((session) =>
    isStackableTransientSession(session, now),
  );
...
function applyVerifiedFocusIfMatched(state: AgentIslandState, now: number): boolean {
...
function shouldShowShadow(
  ...
  return (
    ...
    pointerInside ||
    Boolean(
      current && (current.pendingInteractionIds.size > 0 || current.phase === "needs-interaction"),
    )
  );
}
...
function pillTitle(session: AgentIslandSessionState, mode: "micro" | "compact"): string {
  const title =
    meaningfulSessionTitle(session) ||
    session.projectName ||
...
function compactDetailForSession(session: AgentIslandSessionState): string {
  ...
  if (messagePreview)
    return truncateInlineText(messagePreview, AGENT_ISLAND_COMPACT_DETAIL_MAX_LENGTH);
  ...
  // slice() copies first, so reverse() cannot mutate session state.
  const activity = session.activityLines
    .slice()
    // eslint-disable-next-line eslint-plugin-unicorn/no-array-reverse
    .reverse()
  ...
  if (userActivity)
    return truncateInlineText(userActivity.text, AGENT_ISLAND_COMPACT_DETAIL_MAX_LENGTH);
...
function isGenericRunningStatusDetail(detail: string): boolean {
  const normalized = normalizeInlineText(detail).toLowerCase().replace(/\.+$/g, "").trim();
  return (
...
function shouldDisplaySession(
  ...
  return (
    isSessionVisible(session, now, preserveExpiredTransient) &&
...
function toRemoteUnreadSnapshot(
  unread: AgentIslandRemoteUnreadTerminal,
): AgentIslandSessionSnapshot {
  return {
...
function isWaitingSession(session: AgentIslandSessionState, now: number): boolean {
  return (
    ...
    session.phase === "needs-interaction" ||
    (session.phase === "error" &&
      (isUnreadTerminalWithinIslandTtl(session, now) ||
        (session.errorUntil !== null && session.errorUntil > now)))
  );
}
...
function isUnreadCompletedSession(session: AgentIslandSessionState, now: number): boolean {
  return (
    session.phase === "completed" &&
    (isUnreadTerminalWithinIslandTtl(session, now) ||
      (session.completedUntil !== null && session.completedUntil > now))
  );
}
  ...
  if (request.kind === "permission") {
    ...
    return (
      formatIslandToolDetail(
        request.toolName,
        request.input,
        { wording, requireCommandVisible: true },
        {
...
function appendActivityLine(
  ...
  if (previous && previous.kind === kind && kind !== "user" && kind !== "assistant") {
    ...
    session.activityLines = [...session.activityLines.slice(0, -1), line];
    return line;
  }
  session.activitySeq += 1;
  const line = { id: String(session.activitySeq), kind, text };
  session.activityLines = [...session.activityLines, line].slice(-AGENT_ISLAND_ACTIVITY_MAX_LINES);
  return line;
}
...
function replaceMessagePreviewLine(
  session: AgentIslandSessionState,
  line: AgentIslandActivityLine,
): void {
...
function appendCompletionPlaceholderIfNeeded(
  session: AgentIslandSessionState,
  strings: AgentIslandStrings,
): void {
...
function projectNameFromWorkingDir(
