...
// Ported from Cindy; per-field normalization branches are exhaustive by design.
...
function normalizePreference(raw: unknown): StoredLayoutPreference | null {
  ...
  if (
    typeof record.compactContentWidth === "number" &&
    Number.isFinite(record.compactContentWidth)
  ) {
    preference.compactContentWidth = record.compactContentWidth;
  }
  if (
    typeof record.expandedContentWidth === "number" &&
    Number.isFinite(record.expandedContentWidth)
  ) {
