// Paseo port of Cindy's agent-island shared protocol types.
// Brand + session-activity snapshot are inlined so this module has no
// cross-repo imports. Wire protocol (stdin/stdout NDJSON + IPC channels)
// stays identical to Cindy's helper.

export const PASEO_AGENT_ISLAND_BRAND_NAME = "Paimon";

/** Minimal activity snapshot this island consumes (Paseo adapter fills it). */
export interface PaseoIslandActivityBase {
  sessionId: string;
 ...
 */
export interface AgentIslandSessionActivity extends PaseoIslandActivityBase {
  phase: AgentIslandSessionPhase;
  interactionKind?: AgentIslandInteractionKind;
...
export type AgentIslandDisplayPolicy =
  | "closed"
  | "peek"
  | "blocking"
  | "transient"
  | "manualExpanded";
export type AgentIslandDisplaySurface =
  | "collapsed"
  | "sessionList"
  | "interactionCard"
  | "completionCard";
...
export const DEFAULT_AGENT_ISLAND_MASCOT_SKIN: AgentIslandMascotSkin = "paimon";
...
export function cloneAgentIslandSoundSettings(
  settings: AgentIslandSoundSettings,
): AgentIslandSoundSettings {
  return {
...
export function isAgentIslandMascotSkin(value: unknown): value is AgentIslandMascotSkin {
  return (
    typeof value === "string" && (AGENT_ISLAND_MASCOT_SKINS as readonly string[]).includes(value)
  );
}

export function isAgentIslandSoundId(value: unknown): value is AgentIslandSoundId {
  return (
    typeof value === "string" && (AGENT_ISLAND_SOUND_OPTIONS as readonly string[]).includes(value)
  );
}
...
export function isAgentIslandSoundChoice(value: unknown): value is AgentIslandSoundChoice {
  ...
  return (
    record.type === "custom" && typeof record.path === "string" && record.path.trim().length > 0
  );
}
...
export function isAgentIslandSoundEvent(value: unknown): value is AgentIslandSoundEvent {
  return (
    typeof value === "string" && (AGENT_ISLAND_SOUND_EVENTS as readonly string[]).includes(value)
  );
}

export function cloneAgentIslandDisplayTarget(
  target: AgentIslandDisplayTarget,
): AgentIslandDisplayTarget {
  return target.mode === "display"
    ? {
        ...
        ...(typeof target.displayInternal === "boolean"
          ? { displayInternal: target.displayInternal }
          : {}),
...
export function normalizeAgentIslandDisplayTarget(raw: unknown): AgentIslandDisplayTarget {
  ...
  if (
    ...
    const displayBounds =
      typeof rawBounds === "object" && rawBounds !== null
...
export function normalizeAgentIslandSoundSettings(raw: unknown): AgentIslandSoundSettings {
  ...
  const record = raw as Record<string, unknown>;
  const rawSounds =
    typeof record.sounds === "object" && record.sounds !== null
...
export const DEFAULT_AGENT_ISLAND_STRINGS: AgentIslandStrings = {
  appName: PASEO_AGENT_ISLAND_BRAND_NAME,
  ...
  error: "Error",
  outputLimit:
    "The model reached its output limit, so this response may be incomplete. Send the next message to continue.",
  input: "Input",
  done: "Done",
  running: "Running",
...
export function computeAgentIslandSimulatedNotchWidth(displayWidth: number): number {
  return Math.min(
    ...
    Math.max(
      AGENT_ISLAND_SIMULATED_NOTCH_MIN_WIDTH,
      displayWidth * AGENT_ISLAND_SIMULATED_NOTCH_WIDTH_RATIO,
    ),
  );
}
...
export function getAgentIslandCompactBadgeWidth(input: {
  ...
  return Math.max(minWidth, segmentsWidth + spacing + AGENT_ISLAND_COMPACT_BADGE_CONTENT_INSET * 2);
}
...
export function getAgentIslandDefaultContentWidth(input: {
  ...
  let notchWidth: number;
  if (input.screenMetrics) {
    notchWidth = input.screenMetrics.notchWidth;
  } else if (typeof input.displayWidth === "number") {
    notchWidth = computeAgentIslandSimulatedNotchWidth(input.displayWidth);
  } else {
    notchWidth = AGENT_ISLAND_COMPACT_IDLE_WIDTH;
  }
  if (input.screenMetrics?.hasNotch) {
    ...
    const badgeExtra =
      input.hasSession && input.pillSnapshot
        ? (getAgentIslandCompactBadgeWidth(input.pillSnapshot) +
            AGENT_ISLAND_COMPACT_HARDWARE_BADGE_RESERVED_INSET) *
          2
        : 0;
    return Math.max(AGENT_ISLAND_COMPACT_IDLE_WIDTH, notchWidth + Math.max(baseExtra, badgeExtra));
  }
...
export function snapAgentIslandCompactHardwareContentWidth(input: {
  ...
  const hiddenThreshold =
    baseBasicWidth -
    Math.min(AGENT_ISLAND_COMPACT_HARDWARE_HIDDEN_PULL_DISTANCE, Math.max(24, gap * 0.5));
